...
class HumanSimulatorConfig(BaseModel):
    ...
    def max_greater_than_min_delay(cls, v:float, info:ValidationInfo):
        if 'min_delay' in info.data and v <= info.data['min_delay']:
            raise ValueError('max_delay必须大于min_delay')
        return v

    @field_validator('think_time_max')
    def think_max_greater_than_min(cls, v:float, info:ValidationInfo):
        if 'think_time_min' in info.data and v <= info.data['think_time_min']:
            raise ValueError('think_time_max必须大于think_time_min')
        return v

    @field_validator('speed_max')
    def speed_max_greater_than_min(cls, v, info:ValidationInfo):
        if 'speed_min' in info.data and v <= info.data['speed_min']:
